...
def get_projects_with_scraplists( user, scrapslength=5 ):
    ...
    for project_entity in Project.all().filter('user =', user).order("-updated"):
        ...
        projects.append( {'user':project_entity.user, 'name':project_entity.name, 'scraps':scraps.fetch(scrapslength), 'id':project_entity.key().id(), 'remainder':scraps_remainder} )
